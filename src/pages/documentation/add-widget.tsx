...
const cardFormRows = [
  ["hideErrorMessages", "boolean", "Параметр, с помощью которого можно скрыть ошибки валидации в инпутах"],
  ["hideEmail", "boolean", "Параметр, с помощью которого можно скрыть поле email"],
  ["hideCardContainer", "boolean", "Параметр, с помощью которого можно убрать стили контейнера карты"],
  ["hideResultMessage", "boolean", "Параметр, который скроет результат оплаты. С помощью onSuccess или onError, которые возвращают error_code и error_message, вы сможете отрисовать свой элемент. Если укажете true, фрэйм закроется после оплаты"],
...
const stylesRows = [
  ...
  ["middleContainer", "object", "Объект для изменения стилей среднего контейнера. Полезно для управления отступами"],
...
const AddWidget: React.FC<AddWidgetProps> = () => {
  return (
    <DocumentationPage
      pageName="Добавление виджета"
      pageTitle="Добавление виджета"
    >
      <div className="interface-rest">
        ...
        <p>
          ...
          <br/>
          <CodeBox isInline>{"<script src='https://ioka.kz/data/ioka-widget.js'></script>"}</CodeBox>
        </p>
        <p>
...
:
        ...
        <p>
          <CodeBox isInline>elementId</CodeBox> - ID html элемента, в котором будет рендериться виджет
        </p>
        <p>
          <CodeBox isInline>options</CodeBox> – объект для дополнительных опций (подробности в таблице ниже)
        </p>
        <p>
          <CodeBox isInline>publicKey</CodeBox> – публичный ключ с личного кабинета
        </p>
        <br />
        <p>Параметры объекта <CodeBox isInline>options</CodeBox></p>
        <br/>
        <div className="table-container">
          <Table headRows={headRows} rows={optionRows} className="documentation-table" />
        </div>
        <p><CodeBox isInline>widgetProps</CodeBox> принимает 2 объекта:</p>
        <p>&nbsp;&nbsp;<CodeBox isInline>cardForm</CodeBox>- для кастомизации элементов рендера</p>
        <p>&nbsp;&nbsp;<CodeBox isInline>styles</CodeBox>- для кастомизации стилей</p>
        <br/>
        <p>Параметры объекта <CodeBox isInline>cardForm</CodeBox></p>
        <br/>
        <Table headRows={headRows} rows={cardFormRows} />
        <p>Параметры объекта  <CodeBox isInline>styles</CodeBox></p>
        <br/>
        <Table headRows={headRows} rows={stylesRows} />
        <br />
        <p><a href="/data/example.html" className="link" download>
          Скачать готовые примеры
        </a> для тестирования виджета</p>
      </div>
    </DocumentationPage>
  );
};
