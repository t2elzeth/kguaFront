...
export type PaymentRegisterProps = {};
...
const PaymentRegister: React.FC<PaymentRegisterProps> = () => {
  return (
    <DocumentationPage
      pageName="Регистрация платежей"
      pageTitle="Запрос на регистрацию платежа"
    >
      <div className="interface-rest">
        ...
        <p>Для того чтобы сохранить карту без оплаты, передайте значение “VERIFY” в параметр <a href="#features">features</a> и укажите параметр <a href="#client_id">client_id</a> (id клиента, к которому привязывается карта). </p>
        ...
        <p>Список сохраненных карт клиента можно получить обратившись в метод <CodeBox isInline>/api/cards/</CodeBox> как  описано <a href="/documentation/get-saved-cards" className="link">тут</a></p>
        <br/><br/>
        <h1>Регистрация оплаты с возможностью сохранение карты:</h1>
        <p>
          Если указан параметр <a href="#client_id">client_id</a>, и не указан параметр <a href="#token">token</a>, то в форме оплаты у вашего клиента будет возможность сохранить карточные данные в системе ioka. Сохраненной карте будет присвоен уникальный token. Вы можете принимать оплату без ввода карточных данных, используя token сохраненной карты как указано ниже
        </p>
        <br/><br/>
        <h1>Регистрация оплаты по сохраненной карте (токену):</h1>
        <p>
          Если указана валидная пара <a href="#client_id">client_id</a> и <a href="#token">token</a>, то в форме оплаты будут заполнены ранее сохраненные карточные данные (pan masked, expiry date, holder)
        </p>
        <h1>Регистрация оплаты по сохраненной карте из хранилища MasterPass:</h1>
        <p>
          Для проведения оплаты через карты сохраненные в хранилищи MasterPass необходимо зарегестировать заказ по роуту <CodeBox isInline>/api/payments/register/</CodeBox> с указанием поле features, значения MASTERPASS. При указании что оплата будет проводиться посредством сохраненных в MasterPass  карт, то параметры fingerprint и phone становятся объязательными.
        </p>
      </div>
    </DocumentationPage>
  );
